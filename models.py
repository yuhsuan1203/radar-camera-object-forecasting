...
class EncoderRNN(nn.Module):
    def __init__(self, device, num_hidden, num_layers):
        ...
        self.encoder1 = nn.GRUCell(10, self.num_hidden)
        self.device = device
        self.num_layers = num_layers
        if num_layers > 1:
            self.encoder2 = nn.GRUCell(10, self.num_hidden)
        if num_layers > 2:
            self.encoder3 = nn.GRUCell(10, self.num_hidden)

    def forward(self, input, val=False):
        context = torch.zeros(input.size(0), self.num_hidden, dtype=torch.float).to(self.device)

        #print(input.size())
        #print('first', input.size(0))
        #print('last', input.size()[-1])
...
class DecoderRNN(nn.Module):
    ...
    def forward(self, context, val=False): #dtp_features, val=False):
        outputs = []
